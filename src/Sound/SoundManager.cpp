...
auto SoundManager::load_dig() -> void {
    ...
    l1.clips.push_back(create_refptr<Audio::Clip>(
        TexturePackManager::get().get_file("assets/audio/dig/glass1.wav")));
    l1.clips.push_back(create_refptr<Audio::Clip>(
        TexturePackManager::get().get_file("assets/audio/dig/glass2.wav")));
    l1.clips.push_back(create_refptr<Audio::Clip>(
        TexturePackManager::get().get_file("assets/audio/dig/glass3.wav")));

    dig.emplace("glass", l1);

    std::string prefix = "assets/audio/dig/";
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (int c = 0; c < 4; c++)
            l.clips.push_back(
                create_refptr<Audio::Clip>(TexturePackManager::get().get_file(
...
auto SoundManager::load_step() -> void {
    std::string prefix = "assets/audio/step/";
    std::string name[4] = {"grass", "gravel", "stone", "wood"};
