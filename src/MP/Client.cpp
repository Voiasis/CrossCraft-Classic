...
void Client::process_packet(RefPtr<Network::ByteBuffer> packet) {
    ...
    switch (data->PacketID) {
        ...
        case Incoming::eLevelInitialize: {
            ...
            ringbuffer =
                create_scopeptr<Network::ByteBuffer>(256 * 64 * 256 + 4);
            break;
        }
        ...
        case Incoming::eLevelFinalize: {
            ...
            auto data2 =
                reinterpret_cast<Incoming::LevelFinalize *>(data.get());
            ...
            wrld->chunksMeta = (ChunkMeta *)realloc(
                wrld->chunksMeta,
                wrld->world_size.x / 16 * (wrld->world_size.y / 16 + 1) *
        ...
        case Incoming::eSpawnPlayer: {
            ...
            if (data2->PlayerID == -1) {
                ...
                std::string user =
                    std::string((char *)data2->PlayerName.contents);
                user = user.substr(0, user.find_first_of(0x20));
                player_rep.emplace(
                    data2->PlayerID,
                    PlayerInfo{user, data2->X, data2->Y, data2->Z, data2->Yaw,
                               data2->Pitch});
            }
        ...
        case Incoming::ePlayerTeleport: {
            auto data2 =
                reinterpret_cast<Incoming::PlayerTeleport *>(data.get());
        ...
        case Incoming::ePositionUpdate: {
            auto data2 =
                reinterpret_cast<Incoming::PositionUpdate *>(data.get());
            if (player_rep.find(data2->PlayerID) != player_rep.end()) {
                player_rep[data2->PlayerID].X += data2->DX;
                player_rep[data2->PlayerID].Y += data2->DY;
                player_rep[data2->PlayerID].Z += data2->DZ;
        ...
        case Incoming::eDespawnPlayer: {
            auto data2 =
                reinterpret_cast<Incoming::DespawnPlayer *>(data.get());
            auto id = data2->PlayerID;

            if (player_rep.find(id) != player_rep.end()) player_rep.erase(id);
...
void Client::update(double dt) {
    ...
    for (auto &p : packetsIn) process_packet(p);
    packetsIn.clear();
